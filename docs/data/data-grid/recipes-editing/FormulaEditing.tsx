...
const formulaColumn: Omit<GridColDef, 'field'> = {
  type: 'string',
  editable: true,
...
export default function FormulaEditing() {
  ...
  const onCellModesModelChange = React.useCallback<
    NonNullable<DataGridPremiumProps['onCellModesModelChange']>
  >(
    (model) => {
...
const FUNCTIONS = [
  {
    ...
    transform: (input: string) => {
      return input.replace(/SUM\(([^)]*)\)/g, (match, argsString: string) => {
  ...
  },
  {
    name: 'COUNT',
    transform: (input: string) => {
      return input.replace(/COUNT\(([^)]*)\)/g, (match, argsString: string) => {
        const args = argsString
          .split(',')
          .map((arg) => parseFloat(arg.trim()))
          .filter((num) => !Number.isNaN(num));
